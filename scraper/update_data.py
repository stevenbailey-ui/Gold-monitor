...
Pulls end-of-day prices (Stooq, free), rate series (FRED, free key), merges
low-frequency manual inputs, scores the six-theme gold thesis composite, and
writes data/data.json which the static front end reads.

Runs in CI (GitHub Actions) where the network is open. Every fetch degrades
gracefully: if a source is unreachable, the metric falls back to its last
known value so the dashboard never breaks. Designed for once-daily use.
"""
import csv, io, json, os, sys, urllib.request, datetime, statistics
from pathlib import Path
import config as C

ROOT = Path(__file__).resolve().parents[1]
DATA = ROOT / "data"
UA = {"User-Agent": "gold-monitor/1.0"}
TIMEOUT = 20
...
def stooq_last(symbol):
    """Latest EOD close from stooq."""
    txt = _get(f"https://stooq.com/q/l/?s={symbol}&f=sd2t2ohlcv&e=csv")
    row = list(csv.DictReader(io.StringIO(txt)))[0]
    return float(row["Close"])


def stooq_monthly(symbol, months=30):
    """Monthly closes for trailing-average maths."""
    txt = _get(f"https://stooq.com/q/d/l/?s={symbol}&i=m")
...
def score_metric(meta, value):
    """Return +1 bull / 0 base / -1 bear per the metric's band."""
    if value is None:
        return 0
...
def main():
    ...
    raw = {}      # metric_id -> live value
    notes = []    # fetch failures

    def remember(mid):
        return prev_metrics.get(mid, {}).get("value")

    # gold + 24m trailing first (other ratios depend on it)
    try:
        gold = stooq_last(C.STOOQ["gold"])
    except Exception as e:
        gold = remember("_gold") or 4354.0; notes.append(f"gold:{e}")
    try:
        monthly = stooq_monthly(C.STOOQ["gold"], 24)
    ...
    px = {}  # helper prices for ratios
    for key in ("dxy", "vix", "brent", "gdx", "gdxj", "ndx", "thx", "mtl"):
        try:
            px[key] = stooq_last(C.STOOQ[key])
        except Exception as e:
            px[key] = None; notes.append(f"{key}:{e}")

    # resolve every metric value from its src
    for mid, meta in C.METRICS.items():
        ...
        try:
            ...
            elif src.startswith("stooq:"):
                val = px.get(src.split(":")[1])
            elif src.startswith("stooq_delta:"):
    ...
    # build per-metric records + theme + composite
    metrics_out, theme_score, theme_max = {}, {}, {}
    composite = 0
    for mid, meta in C.METRICS.items():
        v = raw.get(mid)
        sig = score_metric(meta, v)
    ...
    themes_out = []
    for t in C.THEMES:
        themes_out.append({"id": t, "label": C.THEME_LABELS[t], "signal": theme_signal(t), "reading": readings[t]})
    ...
    # current portfolio value = shares x EOD price (pence -> GBP)
    ...
    out = {
        ...
        "holdings": {
            "THX": {"shares": holdings.get("THX"), "price_pence": round(thx_p, 2),
                    "npv": scen.get("npv_per_share", {}).get("THX"),
                    "jurisdiction": manual.get("jurisdiction", {}).get("THX")},
            "MTL": {"shares": holdings.get("MTL"), "price_pence": round(mtl_p, 2),
                    "npv": scen.get("npv_per_share", {}).get("MTL"),
                    "jurisdiction": manual.get("jurisdiction", {}).get("MTL")},
        },
        "scenarios": scen.get("scenarios"),
        "catalysts": manual.get("catalysts"),
        "alerts": manual.get("alerts"),
        "fetch_notes": notes,
    }
    (DATA / "data.json").write_text(json.dumps(out, indent=2))
    print(f"wrote data.json — composite {composite:+d} ({verdict}); {len(notes)} fetch fallbacks")
    if notes:
